  ...
  onSummaryClick(event) {
    ...
    if (detailsElement === this.mainDetailsToggle) {
      ...
      setTimeout(() => {
        ...
        detailsElement.querySelector(".submenu").addEventListener(
          "transitionend",
          () => {
...
class SlideshowComponent extends SliderComponent {
  ...
  linkToSlide(event) {
    ...
    const slideScrollPosition =
      ...
      this.sliderItemOffset * (this.sliderControlLinksArray.indexOf(event.currentTarget) + 1 - this.currentPage)
    this.slider.scrollTo({
...
class VariantSelects extends HTMLElement {
  ...
  updateVariantStatuses() {
    const addButtonText = document.querySelector(`#product-form-${this.dataset.section} .product-form__buttons`)
    const addButton = document.querySelector(`#product-form-${this.dataset.section} .product-form__cart-submit`)
    const addButtonTextDesktop = document.querySelector(
      `#product-form-${this.dataset.section} .product-form__cart-submit span`,
    )
    if (!addButton) return

    this.querySelectorAll(".product-form__buttons").forEach((button) => {
      button.classList.remove("product-form__buttons--add-to-cart-sold-out")
    })
    ...
    if (this.currentVariant.available) {
      ...
      this.querySelectorAll(".product-form__buttons").forEach((button) => {
        button.classList.add("product-form__buttons--add-to-cart-sold-out")
      })
    }
  }
  ...
  renderProductInfo() {
    ...
    fetch(
      ...
      .then((responseText) => {
        ...
        const addButtonCurrent = document
          .getElementById(`product-form-${this.dataset.section}`)
          ?.querySelector(".product-form__buttons")
        if (addButtonUpdated && addButtonCurrent) addButtonCurrent.innerHTML = addButtonUpdated.innerHTML

        const qtyRulesUpdated = html.getElementById(
          `Quantity-Rules-${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`,
        )
        const volumeNoteUpdated = html.getElementById(
          `Volume-Note-${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`,
        )

        if (qtyRules && qtyRulesUpdated) qtyRules.innerHTML = qtyRulesUpdated.innerHTML
        if (volumeNote && volumeNoteUpdated) volumeNote.innerHTML = volumeNoteUpdated.innerHTML
      })
  }
  ...
  setUnavailable() {
    ...
    const addButtonText = document
      .getElementById(`product-form-${this.dataset.section}`)
      ?.querySelector('[name="add"] > span')
    if (!button) return
